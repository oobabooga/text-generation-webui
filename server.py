...
def get_available_models():
    if shared.args.flexgen:
        return sorted([re.sub('-np$', '', item.name) for item in list(Path(shared.args.model_dir+"/").glob('*')) if item.name.endswith('-np')], key=str.lower)
    else:
        return sorted([re.sub('.pth$', '', item.name) for item in list(Path(shared.args.model_dir+"/").glob('*')) if not item.name.endswith(('.txt', '-np', '.pt', '.json'))], key=str.lower)
