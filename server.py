...
def save_prompt(text, prompt_name):
    if prompt_name:
        fname = f"{prompt_name}.txt"
    else:
...
def clear_prompt_name():
    shared.gradio['prompt_name'].update(value="")
...
def create_prompt_menus():
    with gr.Row():
        ...
        with gr.Column():
            with gr.Row():
                shared.gradio['prompt_name'] = gr.Textbox(type='text', lines=1, label='Save Custom Prompt Name')
            with gr.Column(elem_id="PromptSaveRow"):
            ...
            with gr.Tab("Text generation", elem_id="main"):
                with gr.Row():
                    with gr.Column(scale=4):
                        with gr.Tab('Raw'):
                            shared.gradio['textbox'] = gr.Textbox(value=default_text, elem_id="textbox", lines=27, label="Notebook")
