    ...
    with gr.Row():
        with gr.Column():
            with gr.Box():
                ...
                with gr.Row():
                    with gr.Column():
                        shared.gradio['temperature'] = gr.Slider(0.01, 1.99, value=generate_params['temperature'], step=0.01, label='temperature', info='Primary factor to control randomness of outputs. 0 = deterministic (only most likely token is used). Higher value = more randomness.')
                        shared.gradio['top_p'] = gr.Slider(0.0, 1.0, value=generate_params['top_p'], step=0.01, label='top_p', info='If not set to 1, only selects from the most probable tokens with probability adding up to less than this number. Higher value = more range of possible random results.')
                        shared.gradio['top_k'] = gr.Slider(0, 200, value=generate_params['top_k'], step=1, label='top_k', info='Number of most-probable next tokens to randomize between. Higher value = more range of possible random results.')
                        shared.gradio['typical_p'] = gr.Slider(0.0, 1.0, value=generate_params['typical_p'], step=0.01, label='typical_p', info='If not set to 1, only selects sets of possible tokens that are at least this much more likely than random tokens, specifically attempting to factor in context of prior text.')
                    with gr.Column():
                        ...
                        shared.gradio['encoder_repetition_penalty'] = gr.Slider(0.8, 1.5, value=generate_params['encoder_repetition_penalty'], step=0.01, label='encoder_repetition_penalty', info='Exponential penalty factor for tokens that *are not* repeated. Essentially repetition_penalty in reverse. 1 means no penalty, higher value = less likely to generate new tokens, lower value = more likely to generate new tokens.')
                        ...
                        shared.gradio['min_length'] = gr.Slider(0, 2000, step=1, value=generate_params['min_length'] if shared.args.no_stream else 0, label='min_length', interactive=shared.args.no_stream, info='Only valid with `--no-stream` enabled. Minimum generation length in tokens.')
