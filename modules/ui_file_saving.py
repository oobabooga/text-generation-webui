...
def create_ui():
    ...
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['character_saver']:
        shared.gradio['save_character_filename'] = gr.Textbox(lines=1, label='File name', info=f'The character will be saved to your {shared.args.user_data_dir}/characters folder with this base filename.')
        with gr.Row():
    ...
    with gr.Group(visible=False, elem_classes='file-saver') as shared.gradio['preset_saver']:
        shared.gradio['save_preset_filename'] = gr.Textbox(lines=1, label='File name', info=f'The preset will be saved to your {shared.args.user_data_dir}/presets folder with this base filename.')
...
def handle_save_preset_confirm_click(filename, contents):
    try:
        utils.save_file(f"{shared.args.user_data_dir}/presets/{filename}.yaml", contents)
...
def handle_delete_preset_click(preset):
    return [
        f"{preset}.yaml",
        f"{shared.args.user_data_dir}/presets/",
        gr.update(visible=True)
    ]
...
def handle_save_grammar_click(grammar_string):
    return [
        ...
        f"{shared.args.user_data_dir}/grammars/",
        gr.update(visible=True)
    ]
