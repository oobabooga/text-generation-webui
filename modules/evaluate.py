...
def load_past_evaluations():
    if Path(f'{shared.args.user_data_dir}/logs/evaluations.csv').exists():
        df = pd.read_csv(Path(f'{shared.args.user_data_dir}/logs/evaluations.csv'), dtype=str)
...
def save_past_evaluations(df):
    ...
    past_evaluations = df
    filepath = Path(f'{shared.args.user_data_dir}/logs/evaluations.csv')
...
def calculate_perplexity(models, input_dataset, stride, _max_length):
    ...
    if input_dataset == 'wikitext':
        ...
        with open(Path(f'{shared.args.user_data_dir}/training/datasets/{input_dataset}.txt'), 'r', encoding='utf-8') as f:
            text = f.read()
