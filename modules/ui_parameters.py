                ...
                with gr.Column():
                    with gr.Row():
                        with gr.Column():
                            ...
                            with gr.Blocks():
                                shared.gradio['xtc_threshold'] = gr.Slider(0, 0.5, value=generate_params['xtc_threshold'], step=0.01, label='xtc_threshold', info='If there are multiple tokens with predicted probability at least xtc_threshold...')
                                shared.gradio['xtc_probability'] = gr.Slider(0, 1, value=generate_params['xtc_probability'], step=0.01, label='xtc_probability', info='...remove all except the least probable one from sampling, with probability xtc_probability.')
