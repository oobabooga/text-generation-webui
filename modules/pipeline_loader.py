...
from modules.multimodal_utils import get_available_pipelines
...
def load_pipeline(params: dict) -> Tuple[AbstractMultimodalPipeline, str]:
    ...
    for name in available_pipeline_modules:
        try:
            pipeline_modules[name] = import_module(f'modules.pipelines.{name}.pipelines')
        except:
    ...
    logger.critical(f'{log} Please specify a correct pipeline, or stop using multimodal models')
    raise RuntimeError(f'{log} Please specify a correct pipeline, or stop using multimodal models')