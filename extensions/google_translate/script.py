...
def modify_string(string, source, target):
    ...
    blocks = [(str(uuid.uuid4()), m.group(1)) for m in re.finditer(pattern, string)]
    string_without_blocks = string
    for uuid_str, _ in blocks:
    ...
    for uuid_str, block in blocks:
        # Remove leading and trailing whitespaces from each block
        block = block.strip()
        translated_string = translated_string.replace(uuid_str, '```\n' + block + '\n```')
