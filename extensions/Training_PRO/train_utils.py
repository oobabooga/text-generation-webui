...
def split_sentences(text: str, cutoff_len: int):
    ...
    for char in text:
        ...
        if (any(sentence.endswith(delimiter) for delimiter in delimiters) and not
                (prev_char.isupper() and len(sentence) >= 3 and sentence[-3] != ' ') and not
                any(sentence.endswith(abbreviation) for abbreviation in abbreviations)):
        ...
        for edge_idx in edgeindex:
            ...
            for item in sentences[edge_idx:]:
                if totalLength + item['size'] < max_cut:
                    ...
                    if currentSentence.endswith(EOSX_str) and len(currentSentence.strip()) > min_chars_cut or EOSX_str not in currentSentence and len(currentSentence.strip()) > min_chars_cut:
                        sentencelist.append(currentSentence.strip())
