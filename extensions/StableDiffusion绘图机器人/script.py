...
samplers = [
    "DDIM",
    "DPM++ 2M Karras",
]  # TODO: get the availible samplers with http://{address}}/sdapi/v1/samplers
SD_models = [
    "NeverEndingDream"
...
def triggers_are_in(string):
    ...
    return bool(
        re.search(
            "(?aims)(send|mail|message|me|发送|给我)\\b.+?\\b(image|图片|pic(ture)?|photo|snap(shot)?|selfie|meme)s?\\b",
            string,
        )
    )
    ...
    with gr.Accordion("Parameters", open=True, elem_classes="SDAP"):
        with gr.Row():
            address = gr.Textbox(
                ...
                label="Auto1111的WebUI地址",
            )
            modes_list = ["手动", "沉浸/交互", "图画/冒险"]
            mode = gr.Dropdown(
                modes_list,
            ...
            with gr.Column(scale=1, min_width=300):
                manage_VRAM = gr.Checkbox(value=params["manage_VRAM"], label="管理VRAM")
                save_img = gr.Checkbox(
                    value=params["save_img"],
                    label="保留原始图片并在聊天中使用它们",
                )

            force_pic = gr.Button("强制图片回应")
            suppr_pic = gr.Button("中止图片回应")

        with gr.Accordion("生成参数", open=False):
            prompt_prefix = gr.Textbox(
                placeholder=params["prompt_prefix"],
                value=params["prompt_prefix"],
                label="提示前缀（最好用于描述角色的外观）",
            )
            ...
            with gr.Row():
                with gr.Column():
                    width = gr.Slider(
                        256, 768, value=params["width"], step=64, label="宽度"
                    )
                    height = gr.Slider(
                        256, 768, value=params["height"], step=64, label="高度"
                    )
                with gr.Column():
                    sampler_name = gr.Textbox(
                        placeholder=params["sampler_name"],
                        value=params["sampler_name"],
                        label="采样方法",
                        elem_id="sampler_box",
                    )
                    steps = gr.Slider(
                        1, 150, value=params["steps"], step=1, label="采样步骤"
                    )
            with gr.Row():
                seed = gr.Number(label="种子", value=params["seed"], elem_id="seed_box")
                cfg_scale = gr.Number(
                    label="CFG缩放", value=params["cfg_scale"], elem_id="cfg_box"
                )
                with gr.Column() as hr_options:
                    restore_faces = gr.Checkbox(
                        value=params["restore_faces"], label="还原面部"
                    )
                    enable_hr = gr.Checkbox(value=params["enable_hr"], label="高分辨率修复")
            with gr.Row(
                visible=params["enable_hr"], elem_classes="hires_opts"
            ) as hr_options:
                hr_scale = gr.Slider(
                    1, 4, value=params["hr_scale"], step=0.1, label="放大倍数"
                )
